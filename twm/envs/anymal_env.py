...
class ANYmalBEnv(PipelineEnv if _HAS_BRAX else object):
    ...
    def _get_obs_dim(self) -> int:
        # qpos[7:] (12 joints) + qvel[6:] (12 joint vels) + base_z (1) + base_rot (4) + base_linvel (3) + base_angvel (3) + last_act (12)
    ...
    def reset(self, rng: jax.Array) -> Any:
        ...
        nominal_qpos = jnp.array([
            0.0, 0.0, 0.55,          # Root pos (x, y, z)
            1.0, 0.0, 0.0, 0.0,       # Root quat (w, x, y, z)
            0.0, 0.4, -0.8,           # LF leg
            0.0, 0.4, -0.8,           # RF leg
            0.0, -0.4, 0.8,           # LH leg
            0.0, -0.4, 0.8,           # RH leg
